...
class SocketClient:
    def __init__(self, host, port, push_callback=None):
        self.host = host
        self.port = port
        self.sock = None
        self.push_callback = push_callback  # 新增
        self.msg_queues = {
            'heartbeat': Queue(),
    ...
    def _start_aggregation_timer(self):
        ...
        def timer_loop():
            while self.running:
                time.sleep(1)  # 每秒触发一次汇聚
                with self.metrics_lock:
                    data = {
                        'timestamp': int(time.time()),
                        'cpu': self.latest_metrics['cpu'],
                        'memory': self.latest_metrics['memory']
                    }
                    # 重置指标值，避免重复发送旧数据
                    self.latest_metrics.update(cpu=None, memory=None)
                if self.push_callback:
                    self.push_callback(data)
        threading.Thread(target=timer_loop, daemon=True).start()

    def _handle_indicator1(self, body):
        try:
            data = indicator1_struct.parse(body)
            with self.metrics_lock:
                self.latest_metrics['cpu'] = data.value
        except StreamError as e:
            print(f"CPU指标解析失败: {e}")

    def _handle_indicator2(self, body):
        try:
            data = indicator2_struct.parse(body)
            with self.metrics_lock:
                self.latest_metrics['memory'] = data.value
        except StreamError as e:
            print(f"内存指标解析失败: {e}")
